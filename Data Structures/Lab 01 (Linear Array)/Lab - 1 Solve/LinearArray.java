public class LinearArray{
     ...
     public static void reverseArray(int [] source){
          ...
          for (int i = 0 ; i < source.length ; ++ i) {
                source [i] = newArray [newArray.length - 1 - i] ;
          }
     ...
     public static int [] rotateLeft(int [] source, int k){
          ...
          for (int j = source.length - k , p = 0 ; j < source.length ; ++j , ++ p) {
               source[j]= temp[p];
          }
          return source ; // remove this line    
     }
     ...
     public static int [] shiftRight(int [] source, int k){
          for (int index = source.length - 1 ; index >= k ; -- index ) {
               source[index] = source[index - k] ;
          }
          for (int i = 0 ; i < k ; ++ i ) {
               source [i] = 0 ;
     ...
     public static int [] rotateRight(int [] source, int k){
          int[] temp = new int[k];
          for (int j = source.length - 1, p = 0 ; p < temp.length; j-- , p++) {
               temp[p] = source[j];
          }
          for (int index = source.length - 1 ; index >= k  ; --index) {
               source[index] = source[index-k];
          }
          for (int i = 0, z = temp.length - 1; i < k ; ++i , z-- ) {
               source[i]= temp[z];
          }
          return source; // remove this line    
     }
